...
function load_aux_data(T,sz,auxdata_files) # sz= size (lon, lat, dates) matrix
    auxdata = zeros(T,sz[1:end-1]...,2*length(auxdata_files),sz[end]) 
    #zeros of size (lon, lat, 2* lenght of aux *length of dates)
...
function NCData(data; kwargs...)
    ...
    return DINCAE_mod.NCData(lon,lat,datatime,data_full,missingmask;
