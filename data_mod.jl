...
#TRY TO REMOVE SIZE FROM THE LOAD AUX DATA FUNCTION
function load_aux_data(auxdata_files)
    auxdata = zeros(2*length(auxdata_files))
...
mutable struct NCData{T,N }
    ...
    output_ndims::Int
    ndims::Vector{Int}
    #auxdata::Array{T,} # lon,lat,nparam,time
end
...
function NCData(data;  kwargs...)
    lon,lat,datatime,data_full,missingmask,mask = DINCAE_mod.load_gridded_nc(data)

    default_jitter_std = 0.05

    jitter_std = [getp(d,:jitter_std,default_jitter_std) for d in data]
    ndims = [getp(d,:ndims,1) for d in data]
    #auxdata = load_aux_data(auxdata_files)

    return DINCAE_mod.NCData(lon,lat,datatime,data_full,missingmask,ndims;
