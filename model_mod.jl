...
function recmodel4(sz,enc_nfilter,dec_nfilter,skipconnections,l=1; method = :nearest)
    ...
    if l == length(enc_nfilter)
        ...
        inner = Chain(
            Conv(convkernel,enc_nfilter[l] => enc_nfilter[l+1],Flux.relu; pad = 1),
...
!!! note
    ...
    application.
"""
function reconstruct(T, Atype,data_all,fnames_rec;
                     epochs = 1000,
                     batch_size = 50,
    ...
    data_sources = [DINCAE_mod.NCData(
        d,
        train = i == 1,
        #train = false,
        ntime_win = ntime_win,
    ...
    data_iter = [DINCAE_mod.DataBatches(Atype,ds,batch_size) for ds in data_sources]
    ...
    if loss_weights_refine == (1.,)
        ...
        steps = (DINCAE_mod.recmodel4(sz[1:end-2],enc_nfilter,dec_nfilter,skipconnections; method = upsampling_method),
                 DINCAE_mod.recmodel4(sz[1:end-2],enc_nfilter2,dec_nfilter2,skipconnections; method = upsampling_method))
    end
    ...
    #Juno.@profile @time  for e = 1:epochs
        ...
        loss_avg = train_epoch!(MO,DINCAE_mod.PrefetchDataIter(train),lr;
                                clip_grad = clip_grad)
