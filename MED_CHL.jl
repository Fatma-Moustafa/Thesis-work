...
ENV["PYTHON"]=""
#Pkg.build("PyCall")
Pkg.add(["CUDA","PyPlot","Plots", "PyCall", "ChainRulesCore", "ThreadsX", "Flux","Distributions","GeoDatasets", "Glob", "JSON", "LinearAlgebra", "Interpolations", "Random"])
...
fname = joinpath(localdir,"MED_CHL_2018-01-01_2019-12-31.nc")
# filename of the data with added clouds for cross-validation
fname_cv = joinpath(localdir,"MED_CHL_2018-01-01_2019-12-31_add_clouds.nc")
varname = "CHL"

# Results of DINCAE will be placed in a sub-directory under `localdir`
outdir = joinpath(localdir,"Results_MED_CHL_2018-01-01_2019-12-31")
mkpath(outdir)
