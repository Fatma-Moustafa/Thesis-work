...
function loadbatchavg(case,fnames)
    lon,lat,batch_m_true,batch_m_in,batch_m_rec,batch_sigma_rec,mask = DINCAE_utils_mod.loadbatch(case,fnames[1])

    for i = 2:length(fnames)
        lon,lat,batch_m_true_,batch_m_in_,batch_m_rec_,batch_sigma_rec_,mask = DINCAE_utils_mod.loadbatch(case,fnames[i])
...
function summarydineof(dineofrec)
    ...
    if !isfile(fnamesummary)
        ...
        batch_m_rec = DINCAE_utils_mod.gread(dineofrec,missing)[:,end:-1:1,end-ntest+1:end];
...
function RMS_iteration(case; fnameavg = nothing, yl = case.ylim_rms)
    clf();
    RMScv = [DINCAE_utils_mod.cvrms(case,fn) for fn in sort(glob("data*T*.nc"))]
    plot(10*(1:length(RMScv)),RMScv,label="DINCAE");
    xr = xlim()

    if haskey(case,:dineofrec)
        RMSdineof = DINCAE_utils_mod.rmsdineof(case.dineofrec)
        hlines(RMSdineof,xr[1],xr[2],linestyle="--",colors="c",label="DINEOF");
    end

    if fnameavg != nothing
        RMScvavg = DINCAE_utils_mod.cvrms(case,fnameavg)
...
function compare(cases,fnames)
    ...
    fnameavg = fnames[end]
    SSTc = DINCAE_utils_mod.loadall(fnameavg);
    dineofrec = expanduser("~/Data/Med/AVHRR/Data/sst_all_cloudsadded.rec")
    SSTd = reverse(DINCAE_utils_mod.gread(dineofrec,missing),dims = 2);
    ...
    function comparecase(name,fnameavg)
        if endswith(fnameavg,".nc")
            SST = DINCAE_utils_mod.loadall(fnameavg);
        else
            SST = reverse(DINCAE_utils_mod.gread(fnameavg,missing),dims = 2);
        end
...
function taylorplot(fname; dineofrec = expanduser("~/Data/Med/AVHRR/Data/sst_all_cloudsadded.rec"))
    ...
    d = DINCAE_utils_mod.summarydineof(dineofrec)
    dc = summary(fname)
...
function compare_std_around_seasonalaverage(;
    ...
    case = AVHRR_case
    SSTdineof = reverse(DINCAE_utils_mod.gread(dineofrec,missing),dims = 2)
    SSTdincae = DINCAE_utils_mod.loadall(fnameavg);
    ...
    figure(figsize=(10,6))
    SSTdineof_std = @time DINCAE_utils_mod.std_around_seasonalaverage(SSTdineof,SSTtime);
    SSTdincae_std = @time DINCAE_utils_mod.std_around_seasonalaverage(SSTdincae,SSTtime);
    SSTorig_std = @time DINCAE_utils_mod.std_around_seasonalaverage(SSTorig,SSTtime);

    subplot(1,3,1)
    DINCAE_utils_mod.plot_std_around_seasonalaverage(lon,lat,SSTorig_std)
    title("original $(case.varname) std. dev.")

    subplot(1,3,2)
    DINCAE_utils_mod.plot_std_around_seasonalaverage(lon,lat,SSTdincae_std)
    title("DINCAE $(case.varname) std. dev.")

    subplot(1,3,3)
    ima = DINCAE_utils_mod.plot_std_around_seasonalaverage(lon,lat,SSTdineof_std)
...
function post_process()
    ...
    DINCAE_utils_mod.errstat(fnameavg)
    DINCAE_utils_mod.plotres(fnameavg)
...
function watch_post_process(case; path = path)
    ...
    while true
        ...
        sleep(20)
        @show DINCAE_utils_mod.cvrms.(Ref(case),sort(glob("*T*.nc")));
        @show Dates.now()
    end
end
...
function prep_chlor_a_log()
    case = DINCAE_utils_mod.cases("chlor_a")
    case2 = DINCAE_utils_mod.cases("chlor_a_log")
...
function prep_mv2()
    ...
    for d in data
        ...
        newfnames = DINCAE_utils_mod.splitdata(d.filename,split)
...
function testlags(windtype)
    sst_case = DINCAE_utils_mod.cases("sst_t")
    uwnd_case = DINCAE_utils_mod.cases("uwnd")
    vwnd_case = DINCAE_utils_mod.cases("vwnd")
    ...
    #wnd = fun.(uwnd,vwnd)
    DT = 3
    ssta = DINCAE_utils_mod.remove_seasonal_cycle(sst,datatime; DT = DT)
    ...
    CC = DINCAE_utils_mod.testlagssmooth(ssta,wnd,lags,smoothing_jmax)
