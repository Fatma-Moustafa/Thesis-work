...
function reconstruct(Atype,data_all,fnames_rec;
    ...
    @info "Output variablesss:  $output_varnames"
    ...
    if loss_weights_refine == (1.,)
        model = Model(DINCAE_mod.recmodel3(
            sz[1:end-2],
    ...
    #Juno.@profile @time  for e = 1:epochs
        ...
        for (ii,loss) in enumerate(adam(model, DINCAE_mod.PrefetchDataIter(train); gclip = clip_grad, lr = lr))
            #for (ii,loss) in enumerate(adam(model, train; gclip = clip_grad, lr = lr))
            loss_sum += loss
            N += 1
        end
        push!(losses,loss_sum/N)
        println("epochesssss: $(@sprintf("%5d",e )) loss $(@sprintf("%5.4f",losses[end]))")
        flush(stdout)
