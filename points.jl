...
function interpnd!(pos::AbstractVector{<:NTuple{N}},A::KnetArray,vec) where N
    cuA = CuArray(A)
    cuvec = CuArray(vec)
    @cuda DINCAE_mod.interpnd!(pos,cuA,cuvec)
end
...
function DINCAE_mod.sizex(d::PointCloud)
...
function costfun(xrec,xtrue::Vector{NamedTuple{(:pos, :x),Tuple{Tpos,TA}}},truth_uncertain) where TA <: Union{Array{T,N},KnetArray{T,N}} where Tpos <: AbstractVector{NTuple{N,T}} where {N,T}
    ...
    cost = DINCAE_mod.costfun(xrec_interp2[:,:,1:1],xtrue_interp2[:,:,1:1],truth_uncertain)
...
* `auxdata_files`: gridded auxiliary data file for a multivariate reconstruction. 
`auxdata_files` is an array of named tuples with the fields (`filename`, the file name of the NetCDF file, 
`varname` the NetCDF name of the primary variable and `errvarname` the NetCDF name of the expected standard deviation error). 
For example:
...
The file should contain the variables `lon` (longitude), `lat` (latitude), `dtime` (time of measurement) and `id` (numeric identifier, 
only used by post processing scripts) and `dates` (time instance of the gridded field). The file should be in the 
[contiguous ragged array representation]
(https://cfconventions.org/cf-conventions/cf-conventions.html#_contiguous_ragged_array_representation) as specified by the CF convention
 allowing to group data points into "features" (e.g. tracks for altimetry). Every feature can also contain a single data point.
...
function reconstruct_points(
    ...
    values,coord,dtime,id,dates = DINCAE_mod.loaddata(filename,varname)
    ...
    if loss_weights_refine == (1.,)
        println("no refine")
        noutput = 1
        steps = (DINCAE_mod.recmodel4(sz[1:2],enc_nfilter,skipconnections; method = upsampling_method) ,)
        model = DINCAE_mod.StepModel(steps,noutput,loss_weights_refine,truth_uncertain,gamma,
        ...
        steps = (DINCAE_mod.recmodel4(sz[1:2],enc_nfilter,skipconnections; method = upsampling_method),
                 DINCAE_mod.recmodel4(sz[1:2],enc_nfilter2,skipconnections; method = upsampling_method))

        #model1 = DINCAE.Model(chain1,truth_uncertain)
        #model2
        model = StepModel(steps,loss_weights_refine,truth_uncertain,gamma,regularization_L1_beta,regularization_L2_beta)
    end
    ...
    @time for e = 1:epochs
        ...
        for (ii,loss) in enumerate(adam(model, train; gclip = clip_grad, lr = lr))
            ...
            if (ii-1) % 20 == 0
                println("epoch: $(@sprintf("%5d",e )) loss and learning rate $(@sprintf("%5.4f",loss)) $lr")
                flush(stdout)
            end
        end
